...
// DashboardLayout removed — Website uses Outlet-based layout wrapping
...
import { LayerPanel, SceneSavePanel, PropertyPanel } from './panels';
...
// Lazy-loaded tabs (conditionally rendered based on activeTab)
const AIOptimizationTab = lazy(() => import('./tabs/AIOptimizationTab').then(m => ({ default: m.AIOptimizationTab })));
const AISimulationTab = lazy(() => import('./tabs/AISimulationTab').then(m => ({ default: m.AISimulationTab })));
const ApplyPanel = lazy(() => import('./tabs/ApplyPanel').then(m => ({ default: m.ApplyPanel })));

// Lazy-loaded result report panel (conditionally rendered)
const ResultReportPanel = lazy(() => import('./components/ResultReportPanel'));
import type { LayoutResult, FlowResult, CongestionResult, StaffingResult } from './panels/results';

// Lazy-loaded result panels (conditionally rendered in SimulationResultPanels)
const LayoutResultPanel = lazy(() => import('./panels/results/LayoutResultPanel').then(m => ({ default: m.LayoutResultPanel })));
const FlowResultPanel = lazy(() => import('./panels/results/FlowResultPanel').then(m => ({ default: m.FlowResultPanel })));
const CongestionResultPanel = lazy(() => import('./panels/results/CongestionResultPanel').then(m => ({ default: m.CongestionResultPanel })));
const StaffingResultPanel = lazy(() => import('./panels/results/StaffingResultPanel').then(m => ({ default: m.StaffingResultPanel })));
              ...
              {isPanelCollapsed ? <div className="h-full bg-black/80 backdrop-blur-sm border border-white/10 rounded-xl flex flex-col items-center py-4">
                  ...
                  <div className="flex-1 h-full bg-black/80 backdrop-blur-sm border border-white/10 rounded-xl overflow-hidden flex flex-col relative">
                    ...
                    <div className="absolute top-[41px] left-0 right-0 bottom-0 overflow-y-auto overflow-x-hidden">
                      ...
                      <Suspense fallback={<div className="flex items-center justify-center h-32"><Loader2 className="w-5 h-5 animate-spin text-white/40" /></div>}>
